...
- (id)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        UIViewController *controller = [[UIViewController alloc] initWithNibName:@"SMCalloutView" bundle:[NSBundle mainBundle]];
        calloutView = (UIView *)controller.view;
        CGRect tmp = calloutView.frame;
        calloutView.frame = CGRectMake(0, 0, tmp.size.width, tmp.size.height);
...
- (void)presentCalloutFromRect:(CGRect)rect inLayer:(CALayer *)layer ofView:(UIView *)view constrainedToLayer:(CALayer *)constrainedLayer permittedArrowDirections:(SMCalloutArrowDirection)arrowDirections animated:(BOOL)animated {
    ...
    if ([calloutView respondsToSelector:@selector(getAccessoryButton)]){
        UIButton *accessoryButton = [calloutView performSelector:@selector(getAccessoryButton)];
        if (accessoryButton){
            UIGestureRecognizer *recognizer = nil;
            if ([_rightAccessoryView.gestureRecognizers count] > 0){
                recognizer = [_rightAccessoryView.gestureRecognizers objectAtIndex:0];
            }
            _rightAccessoryView = accessoryButton;
            if (recognizer){
                [_rightAccessoryView addGestureRecognizer:recognizer];
            }
        }
    }
    ...
    [calloutView performSelectorOnMainThread:@selector(setBluePOIFromSerializedString:) withObject:self.title waitUntilDone:YES];
    //titleLabel.text = self.title;
    //subtitleLabel.text = self.subtitle;
    
    // size the callout to fit the width constraint as best as possible
    self.$size = [self sizeThatFits:CGSizeMake(constrainedRect.size.width, self.calloutHeight)];
    self.$size = CGSizeMake(calloutView.frame.size.width, calloutView.frame.size.height);
    ...
    self.$origin = calloutOrigin;
    self.$origin = CGPointMake(-calloutView.frame.size.width/2+20, -calloutView.frame.size.height);
